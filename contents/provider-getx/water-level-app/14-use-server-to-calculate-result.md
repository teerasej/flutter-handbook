# ใช้ server คำนวนข้อความผลลัพธ์

## 1. ส่งข้อมูลไปให้ server แบบ json และรับข้อความมาเก็บเป็นตัวแปร
...
class GateCalculatorController extends GetxController {
  ...
  // กำหนดตัวแปรเพื่อเก็บข้อความที่ได้จาก server
  var resultText = "".obs;
  ...
  // ปรับการทำงานของ function ให้เป็น async
  void calculate() async {
    ...
    // กำหนดค่าเริ่มต้นให้กับตัวแปร เป็นการเคลียร์ค่าเก่า
    resultText.value = "";

    // นำตัวแปรมาสร้างข้อมูลแบบ json
    // สังเกตว่าค่า u1, u2,... เรากำหนดเป็นค่า true ไปก่อน
    var body = {
      "lv_inp": lv_inp,
      "ttn_for_inp": ttn_for,
    ...
    // แสดงข้อความ json ที่ส่งไปให้ server ใน console
    print(body);

    // ใช้ GetConnect ส่งข้อมูลไปให้ server
    ...
    // ตรวจสอบว่า server ตอบกลับมาด้วยรหัส http ที่บ่งบอกถึงการทำงานที่ผิดพลาดหรือไม่
    ...
    // แสดงข้อความผลลัพธ์ที่ได้จาก server บนหน้าแอพ​ โดยกำหนดให้กับตัวแปร resultText
...
## 2. นำข้อมูลที่ได้จาก server มาแสดงในหน้าคำนวน
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                // แสดงข้อความผลลัพธ์ที่ได้จาก server
                SizedBox(
                  height: 16,
                ),
